...
from CustomHelper.Helper import (
    generate_doc_result,
    generate_final_doc_results,
    retry_with_delay_async,
)
...
from Util.PAR_Helper import (
    setup_new_document_format,
    parse_result_to_document_format,
    save_document_to_md,
)
from Util.Retriever_setup import parent_retriever
from Util.console_controller import (
...
async def multi_query_generator_node(state: RAG_State):
    ...
    return {"derived_queries": multi_query_result}
...
def retrieve_node(state: RAG_State):
    ...
    retrieve_result = search_vector_store(
        multi_query=multi_queries, retriever=parent_retriever
    )
    print("---MAIN STATE: RETRIEVE OUT VECTOR DB---")

    return {"keys": {"retrieve_result": retrieve_result}}
...
def grade_document(state: RAG_State):
    ...
    grading_chain = grading_documents_chain()
    ...
    for new_query, documents in retrieve_result.items():
        ...
        grade_result = grading_chain.invoke(
            {"question": new_query, "documents": doc_str}
        )
    ...
    return {"keys": {"grading_results": grading_results}}
...
async def think_high_level_outline_node(state: RAG_State):
    ...
    high_level_outline = await THLO_Graph.ainvoke(
        {"original_question": original_query, "derived_queries": derived_queries}
    )
...
async def parallel_execution_node(state: RAG_State):
    ...
    user_input = input(
        "Fast search completed. Continue with Deep Search? (y/n) ")
...
async def composable_search_node(state: RAG_State):
    ...
    full_document_without_conclusion = setup_new_document_format(
    ...
    def prepare_batch_input_data(_sections: List[SectionPlan]) -> list:
        return [
            {
                "input": _section.as_str(),
                "search_result": "",
                "order": _section.order,
                "section_basic_info": _section.as_str_for_basic_info(),
            }
    ...
    _search_graph_batch_results = await project_manager_graph.abatch(
        search_graph_batch_input, {"recursion_limit": 100}
    )
    search_graph_batch_results.extend(_search_graph_batch_results)
    print("---AGENT BATCH END---")

    ordered_results = {
        search_graph_result["order"]: search_graph_result
        for search_graph_result in search_graph_batch_results
    }
    ...
    for order in sorted(ordered_results.keys()):
        ...
        if search_graph_result["final_section_document"]:
            ...
            result_match = re.search(
                r"<result>(.*?)</result>", output, re.DOTALL)
            if result_match:
                document = result_match.group(1).strip()
            else:
                document = output
        # document = search_graph_result["final_section_document"]
        full_document_without_conclusion += parse_result_to_document_format(
            document=document
        )
    print("---GENERATE DOCUMENT DRAFT DONE---")

    conclusion = await conclusion_chain(
        previous_sections_info=full_document_without_conclusion,
        conclusion_section_info=last_conclusion_section.as_str(),
    )
    full_document_draft_display = (
        full_document_without_conclusion + "\n\n\n" + conclusion
    )

    return {
        "document_title": document_title,
        "document_description": document_description,
        "keys": {"full_document_draft_display": full_document_draft_display},
    }
...
async def generate(state: RAG_State):
    ...
    if full_documents_display:
        ...
        asyncio.ensure_future(
            save_document_to_md(
                full_document=full_documents_display, document_title=document_title
            )
        )
        ...
        user_response = input(
            "[y/n] Would you want to save this document in Vector Store?: "
        )
        if user_response == "y":
    ...
    generation = await retry_with_delay_async(
        chain=rag_chain,
        input={"question": user_question, "documents": documents_for_prompt},
        max_retries=3,
        delay_seconds=60.0,
    )

    return {"keys": {"generation": generation, "full_documents": documents_for_prompt}}
...
def build_graph():
    ...
    workflow.add_conditional_edges(
        "grade_documents",
        decide_to_generate,
        {"think_high_level_outline": "parallel_execution", "generate": "generate"},
    )
    workflow.add_conditional_edges(
        "parallel_execution",
        decide_continue,
        {"True": "composable_search", "False": END},
    )
