...
from Tool.CustomSearchFunc_v2 import (
    arxiv_search_v2,
    youtube_search_v2,
    wikipedia_search,
    asknews_search,
...
class SearchAgentGraph:
    ...
    async def __feedback_node(self, state: AgentState):
        ...
        if isinstance(_search_result, PAR_SUCCESS):
            ...
            return {"intermediate_steps": [(agent_action, PAR_SUCCESS(result))]}
        elif isinstance(_search_result, PAR_ERROR):
            return {"intermediate_steps": [(agent_action, _search_result)]}
        else:
...
class WikipediaSearchAgentGraph(SearchAgentGraph):
    def __init__(self):
        super().__init__(
            Custom_WikipediaQueryRun(
                api_wrapper=WikipediaAPIWrapper()), "Wikipedia"
        )
