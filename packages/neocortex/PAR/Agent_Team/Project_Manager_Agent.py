...
from Agent_Team.Member.Common_Search_AgentGraph import (
    TavilySearchAgentGraph,
    WikipediaSearchAgentGraph,
    YoutubeSearchAgentGraph,
    ArxivSearchAgentGraph,
    AskNewsSearchAgentGraph,
)
...
class route(BaseModel):
    ...
    next: Literal[
        ...
        ...,
        description="Select the next agent ONLY ONE that perform action or 'FINISH'. 'FINISH' means you are team finish all job and ready to return result.",
    )
...
def _get_PM_agent():
    PM_Prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                """You are a perfect project manager agent who has successfully led projects by providing clear instructions to team members over a long period.
...
As the Project Manager Agent, strive to coordinate your team members effectively to create a high-quality section of the document.""",
    ...
    PM_chain = (
        ...
        | PM_Prompt
        | PM_llm.bind_tools(tools=[route], tool_choice="route").with_fallbacks(
            [PM_llm.bind_tools(tools=[route], tool_choice="route")] * 3
        )
    )
    return PM_chain
...
def _check_pm_agent_result(_pm_result: Any, _final_section_document: str):
    if isinstance(_pm_result, list) and len(_pm_result) > 0:
        ...
        raise ValueError(
            f"Unexpected agent output: {_pm_result}, type: {type(_pm_result)}"
        )
...
async def _run_search_agent(
    ...
    print(
        f"---{state['order']} PM AGENT CALLED {search_engine.upper()} AGENT---")
    messages = HumanMessage(
        content=f"Hi! I'm PAR Project Manager Agent! {state['instructions']}"
    )
    try:
        agent_result = await search_agent.ainvoke(
            {
                "input": messages.content,
                "section_basic_info": state["section_basic_info"],
            },
            {"recursion_limit": 100},
        )
        extract_agent_result = extract_result(
            agent_result["agent_outcome"].return_values["output"]
        )
        search_result = _transform_search_result(
            search_engine=search_engine, search_result=extract_agent_result
        )
        return {
        ...
        print(
            f"Error Occur at '{search_engine.lower()
                               } agent node'. Detail: {str(e)}"
        )
...
async def run_pm_agent(state: AgentState) -> dict:
    ...
    return _check_pm_agent_result(
        _pm_result=pm_result, _final_section_document=state["final_section_document"]
    )
...
async def asknews_agent_node(state: AgentState) -> dict:
    ...
    return await _run_search_agent(
        state, PAR_Team_Member_Agent_AskNews.get_search_agent_graph(), "AskNews"
    )
...
async def tavily_agent_node(state: AgentState) -> dict:
    ...
    return await _run_search_agent(
        state, PAR_Team_Member_Agent_Tavily.get_search_agent_graph(), "Tavily"
    )
...
async def wikipedia_agent_node(state: AgentState) -> dict:
    ...
    return await _run_search_agent(
        state, PAR_Team_Member_Agent_Wikipedia.get_search_agent_graph(), "Wikipedia"
    )
...
async def youtube_agent_node(state: AgentState) -> dict:
    ...
    return await _run_search_agent(
        state, PAR_Team_Member_Agent_Youtube.get_search_agent_graph(), "Youtube"
    )
...
async def arXiv_agent_node(state: AgentState) -> dict:
    ...
    return await _run_search_agent(
        state, PAR_Team_Member_Agent_ArXiv.get_search_agent_graph(), "ArXiv"
    )
...
def get_pm_graph():
    ...
    workflow.add_conditional_edges(
        "manager", lambda x: x["next"], conditional_map)
