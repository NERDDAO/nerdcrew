...
class Thought(BaseModel):
    ...
    overall_thought: str = Field(
        description="Write my overall thought. Should be English"
    )
    motivation: str = Field(description="My motivation. Should be English")
    desired_information: str = Field(
        description="My desired information. Should be English"
    )
    gaps_or_limitations: str = Field(
        description="My gaps or limitations part. Should be English"
    )
    document_structure: str = Field(
        description="My document structure part. Should be English"
    )
...
class SectionContent(BaseModel):
    ...
    overview: str = Field(
        ..., description="A brief overview of the section. Should be English"
    )
    key_points: List[str] = Field(
        ...,
        description="The main points to be covered in the section. Should be English",
    )
    description: str = Field(
        ...,
        description="Provide a description of the high-level plan for this overview. Should be English",
    )
...
class Section(BaseModel):
    ...
    order: int = Field(
        ...,
        description="Specify the position of this section in the document's overall structure. Should be English",
    )
    importance: str = Field(
        ...,
        description="Indicate the importance or priority of this section (high, medium, low). Should be English",
    )
...
class HighLevelDocument_Outline(BaseModel):
    ...
    title: str = Field(
        ..., description="The title of the completed document. Should be English"
    )
    objective: str = Field(
        ...,
        description="State the main objective or purpose of the document. Should be English",
    )
    sections: List[Section] = Field(
        ..., description="LIST-UP the sections of the document. Should be English"
    )
    ...
    def as_str(self) -> str:
        ...
        for section in self.sections:
            ...
            for content in section.content:
                ...
                xml_output += f"<description>{
                    content.description}</description>\n"
                xml_output += (
                    f"<expected_outcome>{
                        content.expected_outcome}</expected_outcome>\n"
                )
                xml_output += f"<relation_to_other_sections>{
                    content.relation_to_other_sections}</relation_to_other_sections>\n"
                xml_output += "</item>\n"
            xml_output += "</content>\n"
...
class SectionPlan(BaseModel):
    ...
    title: str = Field(...,
                       description="The name of the section. Should be English")
    order: int = Field(
        ...,
        description="Specify the position of this section in the document's overall structure. Should be English",
    )
    explanation: str = Field(
        ..., description="The explanation of the section. Should be English"
    )
    ...
    outline: str = Field(
        ...,
        description="Provide a detailed outline of the section's structure and content flow. Should be English",
    )

    key_points: List[str] = Field(
        ...,
        description="List the key points to be covered in this section. Should be English",
    )
...
class HighLevelDocument_Plan(BaseModel):
    ...
    sections: List[SectionPlan] = Field(
        ...,
        description="Provide detailed plans for each section of the document. Should be English",
    )
