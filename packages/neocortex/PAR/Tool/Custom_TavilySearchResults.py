...
class TavilyInput(BaseModel):
    ...
    query: str = Field(
        description="The search query to look up using the Tavily Search API."
    )
    max_results: int = Field(
        default=3,
        ge=2,
        le=6,
        description="The maximum number of search results to return. Must be between 2 and 6. Default value is 3",
    )
...
class Custom_TavilySearchAPIWrapper(TavilySearchAPIWrapper):
    ...
    def results(
        ...
        if include_answer:
            return self.clean_results(
                ...
                raw_search_results.get("answer"),
                raw_search_results.get("follow_up_questions"),
                raw_search_results.get("images"),
            )
        else:
    ...
    def clean_results(
        ...
        for result in results:
            clean_result = {"url": result["url"], "content": result["content"]}
...
def get_tavily_search_tool(max_results: Optional[int] = None) -> RunnableWithFallbacks:
    ...
    _tavily_search_tool_with_fallbacks = _tavily_search_tool.with_fallbacks(
        [_tavily_search_tool] * 60
    )
