...
def extract_result(text):
    ...
    if isinstance(text, str):
        ...
        match_section_content = re.search(
            pattern_section_content, text, re.DOTALL)
        match_section_thought = re.search(
            pattern_section_thought, text, re.DOTALL)
        match_section_complete = re.search(
            pattern_section_complete, text, re.DOTALL)
        if match_result:
        ...
        raise TypeError(
            f"Text must be str or unicode, not {type(text)}, Text: {str(text)}"
        )


def parse_result_to_document_format(
    document: dict | FinalResponse_SectionAgent | str,
) -> str:
...
def setup_new_document_format(
    document_title: str, document_description: str, original_question: str
) -> str:
...
async def save_document_to_md(full_document: str, document_title: str):
    ...
    user_response = input(
        "[y/n] Would you want to save this document as a .md file?: ")
