...
def _get_content_extraction_agent() -> RunnableSerializable:
    ...
    content_extraction_prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                """You are an information extraction specialized agent with extensive experience in collecting data from raw content.
...
Follow the guidelines and restrictions mentioned above to contribute effectively to the PAR project.""",
    ...
    content_extraction_llm = get_openai_model()
...
async def youtube_search_v2(
    ...
    youtube_extract_results = await retry_with_delay_async(
        ...
        input={"search_query": query, "search_results": youtube_results},
        max_retries=5,
        delay_seconds=60.0,
    )
...
async def arxiv_search_v2(
    ...
    def convert_arxiv_raw_data_to_raw_content(doc) -> str:
        return (
            f"<arxiv paper title>{doc.metadata['Title']}</arxiv paper>\n"
            f"<arxiv paper published>{
                doc.metadata['Published']}</arxiv paper>\n"
    ...
    arxiv_batch_input = [
        {
            "search_query": query,
            "search_result": convert_arxiv_raw_data_to_raw_content(doc),
        }
        for doc in docs
    ]
    _content_extraction_agent = _get_content_extraction_agent()
