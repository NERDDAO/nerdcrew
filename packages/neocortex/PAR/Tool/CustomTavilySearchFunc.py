...
def _get_proposal_visualization_chain():
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                """You are a data visualization specialized agent with extensive experience in creating effective visual representations of information.
...
def _get_visualization_code_chain():
    class visualization_response(BaseModel):
        ...
        type: Literal["markdown", "mermaid", "matplotlib"] = Field(
            description="visualization code syntax type"
        )
        code: str = Field(
            description="visualization code, do not include backtick(`)")
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                """You are a data visualization specialist tasked with creating visual elements based on the visualization proposals provided by the PAR project team.
...
def _build_tavily_results(search_results: dict) -> str:
    ...
    if (
        isinstance(search_results.get("follow_up_questions", []), list)
        and len(search_results.get("follow_up_questions", [])) > 0
    ):
        web_results += f"<recommended_follow_up_questions>\n"
        for index, follow_up_question in enumerate(
            search_results["follow_up_questions"], start=1
        ):
            web_results += f"{index}. {follow_up_question}\n"
        web_results += "</recommended_follow_up_questions>\n"

    if isinstance(search_results.get("images", []), list) and search_results.get(
        "images"
    ):
        print(
            f"<tavily_has_images>\n{
...
async def tavily_search_func(
    ...
    tavily_search_tool_with_fallbacks = get_tavily_search_tool(
        max_results=max_results)

    try:
        search_results = await tavily_search_tool_with_fallbacks.ainvoke(
            {"query": query + "."}
        )
