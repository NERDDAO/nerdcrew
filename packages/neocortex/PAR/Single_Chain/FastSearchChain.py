...
async def get_fast_search_result(original_question: str) -> str:
    prompt = ChatPromptTemplate.from_template(
        """I'm going to give you a document. Then I'm going to ask you a question about it. I'd like you to first write down exact quotes from the document that would help answer the question, and then I'd like you to answer the question using facts from the quoted content.
    ...
    multiple_search_chain = RunnableParallel(
        tavily=tavily_search_tool_with_fallbacks)
    multiple_search_results = multiple_search_chain.invoke(
        {"query": original_question})

    web_results = {
        "tavily": _build_raw_contents_tavily(
            multiple_search_results["tavily"]["results"]
        ),
    }
    ...
    batch_input = [
        {"search_query": original_question,
            "search_result": web_results["tavily"]},
    ]
    ...
    final_result = chain.invoke(
        {"document": batch_results, "question": original_question}
    )
    return final_result
