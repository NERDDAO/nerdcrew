...
from Tool.CustomSearchFunc import (
    web_search,
    wikipedia_search,
    youtube_search,
    arXiv_search,
)
from Tool.CustomSearchTool import (
    Custom_WikipediaQueryRun,
    Custom_YouTubeSearchTool,
    Custom_arXivSearchTool,
)
from Tool.Respond_Agent_Section_Tool import (
    FinalResponseTool,
    FinalResponse_SectionAgent,
)
...
tools = [tavilytools, wikipedia, youtube_search_tool,
         arXiv_search_tool, response_tool]
chain = (
    {
    ...
    | agent_prompt.partial(
        add_more_restrictions="3. ALWAYS use 'Final-Respond' tool only when all the information about the section has been collected and you are ready to finally inform the user of all the information about the section. If you do not use this tool, you will not be able to forward the results to users, and so you will be penalized."
    )
...
def run_agent(data):
    ...
    agent_outcome = chain.invoke(
        {"section_plan": section_plan, "intermediate_steps": intermediate_steps}
    )
...
def execute_tools(data):
    ...
    if tool_name == "tavily_search_results_json":
        ...
        return {
            "keys": {"tool": "section_complete", "tool_input": agent_action.tool_input}
        }
    else:
...
def tavily_search_node(data):
    ...
    return {"intermediate_steps": [(agent_action, result)]}
...
def wikipedia_search_node(data):
    ...
    return {
        "intermediate_steps": [
            (agent_action, wikipedia_search(
                data["keys"]["tool_input"]["query"]))
        ]
    }
...
def youtube_search_node(data):
    ...
    return {
        "intermediate_steps": [
            (agent_action, youtube_search(data["keys"]["tool_input"]["query"]))
        ]
    }
...
def arXiv_search_node(data):
    ...
    return {"intermediate_steps": [(agent_action, arXiv_result)]}
...
workflow.add_conditional_edges(
    "agent", router, {"end": END, "tools": "action", "retry": "retry"}
)
workflow.add_conditional_edges(
    "action",
    router_tool,
