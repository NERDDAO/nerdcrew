...
def _parse_generate_new_question_chain(generated_prompt: str) -> str:
    generated_prompt_match = re.search(
        r"<generated_prompt>(.*?)</generated_prompt>", generated_prompt, re.DOTALL
    )
...
async def get_generate_new_prompt(
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                """You will be assisting with a RAG (Retrieval-Augmented Generation) AI system that generates new search questions based on user questions. Your task is to take a user's question, translate it into English if needed, identify the key information sought, and generate a new prompt in English based on that key information.
...
""",
    ...
    _generate_new_question_chain = (
        prompt.partial(current_date=current_date) | llm | StrOutputParser()
    )
