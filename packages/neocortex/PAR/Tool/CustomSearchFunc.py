...
from Tool.Custom_TavilySearchResults import (
    Custom_TavilySearchResults,
    Custom_TavilySearchAPIWrapper,
)
...
llm_sonnet = get_openai_model()  # For fallback


extract_tavily_chain = (
    extractor_prompt | llm.with_fallbacks([llm_sonnet]) | StrOutputParser()
)


def web_search(query: str, datastore: BaseStore, retriever: ParentDocumentRetriever):
    ...
    try:
        ...
        if isinstance(search_results, dict) and "results" in search_results:
            ...
            web_results = f"<web_results>\n<overall_summary>{
                search_results['answer']}</overall_summary>"
            chain_batch_input = [
                {"question": query, "context": doc["raw_content"]} for doc in docs
            ]
            chain_batch_result = extract_tavily_chain.batch(chain_batch_input)

            for index, result in enumerate(chain_batch_result):
                web_results += (
                    f"<document index='{index + 1}'>\n"
...
def youtube_search(query: str) -> str:
    ...
    def process_video(index, video):
        try:
            loader = YoutubeLoader.from_youtube_url(
                f"https://www.youtube.com{video['url_suffix']}", add_video_info=True
            )
            load = loader.load()
            ...
            print(
                f"Error occurred while processing video: {
                    video['url_suffix']}"
            )
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [
            executor.submit(process_video, index, video)
...
arxiv_extract_chain = arxiv_extract_prompt.partial(
    restrictions="1. *ALWAYS* use 'ArxivPaperRelevance' tools for your final result.\n2. Take a careful at the schema of the tool, and use the tool. ArxivPaperRelevance input parameter is quite complex, so be careful to use it!"
) | llm.with_structured_output(ArxivPaperRelevance).with_fallbacks(
    [llm_sonnet.with_structured_output(ArxivPaperRelevance)]
)
...
def arXiv_search(
    ...
    arxiv_batch_input = [
        {
            ...
            "provided_section": provided_section,
            "arxiv_document_content": doc.page_content,
        }
        for doc in docs
    ]
    ...
    for index, batch in enumerate(arxiv_batch_result, start=0):
        ...
        if isinstance(batch, ArxivPaperRelevance):
            print(
                f"---REFINE ARXIV DOC{index}-TYPE:ARXIVPAPERRELEVANCE DONE---")
