...
def select_prompt_template(agent_specific_role: str) -> dict:
    if agent_specific_role.lower() == "tavily":
        ...
        search_engine_description = (
            "The arXiv Search API enables you to search for academic papers on the arXiv preprint repository based on a provided search query. By inputting a search term or "
        ...
        search_engine_description = (
            "Use Brave Search when you need a broad and comprehensive search across a wide range of websites and domains. It is particularly effective for finding the most up-to-date information on current events, trending topics, and rapidly evolving fields. "
        ...
        raise ValueError(
            f"Unrecognized agent specifier '{
                agent_specific_role}'"
        )
    return {
...
def create_agent(llm: ChatOpenAI, tool: BaseTool, agent_specific_role: str):
    fallback_llm = get_openai_model()
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                """You are a seasoned researcher agent who has been conducting research using the {search_engine} Search API.
...
""",
    ...
    search_template = select_prompt_template(
        agent_specific_role=agent_specific_role)
    run_llm = llm.bind_tools(tools=[tool]).with_fallbacks(
        [fallback_llm.bind_tools(tools=[tool])] * 3
    )
    agent_chain = (
        {
        ...
        | prompt.partial(
            search_engine=search_template["search_engine"],
            search_engine_description=search_template["search_engine_description"],
            search_query_tip=search_template["search_query_tip"],
        )
        | run_llm
    )
    return agent_chain
